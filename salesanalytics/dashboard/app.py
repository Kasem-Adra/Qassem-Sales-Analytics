...
from salesanalytics.core import load_sales_data, sales_by_dimension, sales_by_month, summarize_sales
from salesanalytics.kpis import month_over_month_growth
from salesanalytics.ml.forecast import moving_average_forecast


st.set_page_config(page_title="Qassem Sales Analytics", layout="wide")
st.title("Qassem Sales Analytics")
st.caption("Professional sales analytics dashboard by Kasem Adra")

uploaded = st.file_uploader("Upload sales data", type=["csv", "xlsx", "xls", "json"])
...
if uploaded:
    ...
    st.dataframe(month_over_month_growth(df), use_container_width=True)

    for dimension in ["region", "product"]:
        normalized = [str(c).strip().lower().replace(" ", "_").replace("-", "_") for c in df.columns]
    ...
    st.dataframe(moving_average_forecast(df, periods=periods), use_container_width=True)
else:
    st.info("Upload a sales file with at least order_date and sales columns.")
