...
from .core import load_sales_data, sales_by_dimension, sales_by_month, summarize_sales, top_items
...
def build_parser() -> argparse.ArgumentParser:
    ...
    contribution = sub.add_parser("contribution", help="Show revenue contribution by dimension.")
    contribution.add_argument("file")
    contribution.add_argument("dimension")

    forecast = sub.add_parser("forecast", help="Generate moving-average forecast.")
    forecast.add_argument("file")
    forecast.add_argument("--periods", type=int, default=3)

    report = sub.add_parser("report", help="Generate standalone HTML report.")
    report.add_argument("file")
    report.add_argument("--output", "-o", default="reports/sales_report.html")

    dashboard = sub.add_parser("dashboard", help="Print command for launching Streamlit dashboard.")
...
def main() -> int:
    ...
    if args.command == "top":
        print_frame(top_items(df, args.dimension, limit=args.limit))
        return 0
    ...
    if args.command == "forecast":
        print_frame(moving_average_forecast(df, periods=args.periods))
        return 0
